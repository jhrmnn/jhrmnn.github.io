#!/usr/bin/env python3
"""Federate changed posts via our own ActivityPub Worker instead of Bridgy Fed.
...
Best-effort, like `send_webmentions.py`: federation problems are logged but never
fail the build. Skips quietly if `AP_ADMIN_TOKEN` is unset (transition period
before the self-hosted actor is configured).
"""
import argparse
import os
import sys
